...
export async function onRequest(context) {
    try {
        ...
        if (!authAccess || authAccess === '0' || context.request.url.includes('/login') || context.request.url.includes('/sendcode') || context.request.url.includes('/test-db') || context.request.url.includes('/auth/') || context.request.url.includes('/debug-schema')) {
            console.log('跳过权限校验');
